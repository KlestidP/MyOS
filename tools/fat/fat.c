...
bool compareFilenames(const uint8_t* entryName, const char* inputName) {
    for (int i = 0; i < 11; i++) {
        if (tolower(entryName[i]) != tolower(inputName[i])) {
            return false;
        }
    }
    return true;
...
DirectoryEntry* fileEntry(const char* name) {
    for (int i = 0; i < g_BootSector.DirEntryCount; i++) {
        if (compareFilenames(g_RootDirectory[i].Name, name)) {
